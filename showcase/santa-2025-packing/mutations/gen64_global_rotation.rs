//! Evolved Packing Algorithm - Generation 64 GLOBAL ROTATION
//!
//! MUTATION STRATEGY: GLOBAL ROTATION OPTIMIZATION
//! After SA, rotate the entire packing to find the angle that
//! minimizes the bounding box side length.
//!
//! Key insight from 70.1 solution analysis:
//! - Top solutions use global rotation optimization
//! - Test rotations 0-90 degrees in 3-degree increments
//! - Can significantly reduce bounding box
//!
//! Changes from Gen62:
//! - Add optimize_rotation() that rotates entire packing
//! - Tests 31 angles (0, 3, 6, ..., 90 degrees)
//! - Applied after SA optimization for each n
//! - Also includes Gen62's radius compression moves
//!
//! Target: Better utilization of square bounding box
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // NEW: Apply global rotation optimization
                self.optimize_rotation(&mut trees);
    ...
    // Rotates entire packing to find angle that minimizes bounding box
    fn optimize_rotation(&self, trees: &mut Vec<PlacedTree>) {
        if trees.len() <= 1 {
            return;
        }

        // Find center of current packing
        let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
        let center_x = (min_x + max_x) / 2.0;
        let center_y = (min_y + max_y) / 2.0;

        let mut best_side = compute_side_length(trees);
        let mut best_angle: f64 = 0.0;

        // Test rotations from 0 to 90 degrees in 3-degree increments
        // Due to symmetry, we only need to test 0-90
        for angle_deg in (0..=90).step_by(3) {
            ...
            for tree in trees.iter() {
                // Rotate tree position around center
                let dx = tree.x - center_x;
                let dy = tree.y - center_y;
                let rot_x = center_x + dx * cos_a - dy * sin_a;
                let rot_y = center_y + dx * sin_a + dy * cos_a;

                // Create rotated tree (position and angle both rotated)
                let rot_angle = (tree.angle_deg + angle_deg as f64).rem_euclid(360.0);
                let rot_tree = PlacedTree::new(rot_x, rot_y, rot_angle);

                // Get bounds of rotated tree
                let (bx1, by1, bx2, by2) = rot_tree.bounds();
                rot_min_x = rot_min_x.min(bx1);
                rot_min_y = rot_min_y.min(by1);
                rot_max_x = rot_max_x.max(bx2);
                rot_max_y = rot_max_y.max(by2);
            }

            let rot_side = (rot_max_x - rot_min_x).max(rot_max_y - rot_min_y);
            if rot_side < best_side {
                best_side = rot_side;
        ...
        if best_angle > 0.0 {
            ...
            for tree in trees.iter_mut() {
                ...
                let rot_x = center_x + dx * cos_a - dy * sin_a;
                let rot_y = center_y + dx * sin_a + dy * cos_a;
                let rot_angle = (tree.angle_deg + best_angle).rem_euclid(360.0);
                *tree = PlacedTree::new(rot_x, rot_y, rot_angle);
            }
        }
    }

    // Radius-based compression move
