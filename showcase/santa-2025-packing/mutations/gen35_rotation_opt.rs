//! Evolved Packing Algorithm - Generation 35 ROTATION OPTIMIZATION
//!
//! MUTATION STRATEGY: Dedicated rotation optimization pass.
//!
//! Changes from Gen28:
//! - After SA, run a rotation optimization pass
//! - For each tree, try all 8 angles and keep best
//! - This can find better orientations SA might miss
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // ROTATION OPTIMIZATION: Try all angles for each tree
                self.rotation_optimization(&mut trees);
    ...
    /// Rotation optimization: try all 8 angles for each tree, keep best
    fn rotation_optimization(&self, trees: &mut Vec<PlacedTree>) {
        ...
        while improved && passes < max_passes {
            ...
            for idx in 0..trees.len() {
                ...
                let mut best_angle = old_tree.angle_deg;
                let mut best_side = current_side;

                for &angle in &angles {
                    if (angle - old_tree.angle_deg).abs() < 0.001 { continue; }

                    let candidate = PlacedTree::new(old_tree.x, old_tree.y, angle);

                    // Check for overlap
                    let valid = trees.iter().enumerate().all(|(i, t)| {
                        i == idx || !candidate.overlaps(t)
                    });

                    if valid {
                        trees[idx] = candidate;
                        let new_side = compute_side_length(trees);
                        if new_side < best_side - 0.0001 {
                            best_side = new_side;
                            best_angle = angle;
                            improved = true;
                        }
                ...
                // Apply best angle
                if (best_angle - old_tree.angle_deg).abs() > 0.001 {
                    trees[idx] = PlacedTree::new(old_tree.x, old_tree.y, best_angle);
                }
            }
        }
    }
