//! Evolved Packing Algorithm - Generation 32A SWAP MOVES
//!
//! MUTATION STRATEGY: Add swap moves between boundary trees.
//!
//! Changes from Gen28:
//! - 10% of moves try to swap positions of two boundary trees
//! - This can help break out of local optima by larger-scale restructuring
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            elite_pool_size: 3,
            swap_move_prob: 0.10,  // 10% swap moves
        }
    }
}
...
impl EvolvedPacker {
    ...
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, _strategy: PlacementStrategy, rng: &mut impl Rng) {
        ...
        for iter in 0..base_iterations {
            ...
            // TRY SWAP MOVE
            let do_swap_move = rng.gen::<f64>() < self.config.swap_move_prob && boundary_info.len() >= 2;

            if do_swap_move {
                // Try swapping two boundary trees
                let old_trees = trees.clone();
                if self.try_swap_move(trees, &boundary_info, rng) {
    ...
    // NEW: Try to swap positions of two boundary trees
    fn try_swap_move(&self, trees: &mut [PlacedTree], boundary_info: &[(usize, BoundaryEdge)], rng: &mut impl Rng) -> bool {
        if boundary_info.len() < 2 { return false; }

        let idx1 = boundary_info[rng.gen_range(0..boundary_info.len())].0;
        let mut idx2 = boundary_info[rng.gen_range(0..boundary_info.len())].0;

        // Make sure we pick two different trees
        let mut attempts = 0;
        while idx1 == idx2 && attempts < 10 {
            idx2 = boundary_info[rng.gen_range(0..boundary_info.len())].0;
            attempts += 1;
        }
        if idx1 == idx2 { return false; }

        // Swap positions (keep angles the same)
...
fn has_any_overlap(trees: &[PlacedTree]) -> bool {
    for i in 0..trees.len() {
        for j in (i+1)..trees.len() {
            if trees[i].overlaps(&trees[j]) { return true; }
        }
    }
    false
}
