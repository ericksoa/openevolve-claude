//! Evolved Packing Algorithm - Generation 29A CROSSOVER: Hot Restarts + Small-n Heavy
//!
//! CROSSOVER STRATEGY:
//! Parent 1: Gen28 Hot Restarts (champion: 90.0)
//!   - Elite pool tracking
//!   - Hot restart when stagnant
//!   - Temperature reset mechanism
//!
//! Parent 2: Gen25 Small-n Heavy (close: 91.65)
//!   - 4x compute for n <= 20
//!   - 3x compute for n <= 40
//!   - More SA passes for small n
//!
//! Combined: Apply hot restarts WITH n-scaled compute allocation.
//! Small n gets more iterations AND hot restarts for better optimization.
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            sa_iterations: 22000,         // Base (scaled by n)
            ...
            fill_move_prob: 0.15,
            // Hot restart parameters from Gen28
...
impl EvolvedPacker {
    ...
    fn get_sa_iterations(&self, n: usize) -> usize {
        if n <= 20 {
            self.config.sa_iterations * 4
        } else if n <= 40 {
            self.config.sa_iterations * 3
        } else if n <= 70 {
            self.config.sa_iterations * 2
        } else if n <= 120 {
    ...
    fn find_placement_with_strategy(
        ...
        // FROM GEN25: N-scaled search attempts
    ...
    // CROSSOVER: Local search with hot restarts AND n-scaled iterations
    fn local_search(
        &self,
        ...
        if is_fill_move {
            ...
            match move_type {
                ...
                8 => {
                    ...
                    trees[idx] = PlacedTree::new(old_x + (cx - old_x) * self.config.center_pull_strength * (0.5 + temp),
                                                  old_y + (cy - old_y) * self.config.center_pull_strength * (0.5 + temp), old_angle);
                }
