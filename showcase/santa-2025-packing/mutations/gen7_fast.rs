//! Evolved Packing Algorithm - Generation 7 FAST PROTOTYPE
//!
...
//!
//! MUTATION STRATEGY: FAST PROTOTYPE (Gen7)
//! Much faster execution to enable more experimentation:
//!
//! Key changes from Gen6:
//! - sa_iterations: 8000 (reduced from 28000) - 3.5x faster
//! - search_attempts: 150 (reduced from 280)
//! - direction_samples: 36 (reduced from 72)
//! - sa_passes: 1 (reduced from 2)
//! - Aggressive early exit: 600 iterations (reduced from 1800)
//! - Keep core density features for quality
//!
//! Target: Fast iteration while maintaining reasonable quality
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen7 FAST PROTOTYPE: Speed-optimized configuration
        Self {
            search_attempts: 150,             // Reduced from 280 for speed
            direction_samples: 36,            // Reduced from 72 for speed
            sa_iterations: 8000,              // Reduced from 28000 - 3.5x faster!
            sa_initial_temp: 0.50,            // Slightly higher for faster convergence
            sa_cooling_rate: 0.9998,          // Faster cooling for fewer iterations
            sa_min_temp: 0.00005,             // Higher minimum for faster termination
            translation_scale: 0.06,          // Slightly larger moves for faster progress
            rotation_granularity: 45.0,       // Keep 8 angles
            center_pull_strength: 0.08,       // Slightly stronger pull for faster compaction
            sa_passes: 1,                     // Single pass for speed!
            early_exit_threshold: 600,        // Aggressive early exit (from 1800)
            boundary_focus_prob: 0.85,        // Keep 85% boundary focus
            // DENSITY parameters (kept from Gen6)
            density_grid_resolution: 16,      // Slightly smaller grid for speed
            gap_penalty_weight: 0.15,         // Keep gap penalty
            local_density_radius: 0.5,        // Keep local density radius
            fill_move_prob: 0.12,             // Slightly reduced fill moves
        }
    }
}
...
impl EvolvedPacker {
    ...
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            // Run SA pass with density-aware moves (single pass for speed)
            self.local_search(&mut trees, n, 0, &mut rng);
    ...
    /// FAST: Reduced iterations with aggressive early exit
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, _pass: usize, rng: &mut impl Rng) {
        ...
        // FAST: Reduced iteration count
        let base_iterations = self.config.sa_iterations + n * 40;
        ...
        for iter in 0..base_iterations {
            // FAST: Aggressive early exit
            if iterations_without_improvement >= self.config.early_exit_threshold {
                break;
            }

            // Update boundary cache every 250 iterations (more frequent for shorter runs)
