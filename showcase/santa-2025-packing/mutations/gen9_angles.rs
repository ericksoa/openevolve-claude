//! Evolved Packing Algorithm - Generation 9 MORE ANGLES
//!
...
//!
//! MUTATION STRATEGY: MORE ANGLES (Gen9)
//! Try more rotation angles for better packing:
//!
//! Key improvements from Gen6:
//! - rotation_granularity: 30.0 (12 angles instead of 8)
//! - select_angles(): return 12 angles based on n % 12
//! - Include 0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330
//! - More rotation options may find better fits in tight spaces
//! - Keep other parameters the same as Gen6 champion
//!
//! Target: Beat Gen6's 94.14 at n=200 with more rotation options
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen9 MORE ANGLES: 12 rotation angles instead of 8
        Self {
            ...
            rotation_granularity: 30.0,      // MORE ANGLES: 12 angles (changed from 45.0)
            ...
            // DENSITY parameters (same as Gen6)
            density_grid_resolution: 20,     // 20x20 grid for density tracking
            gap_penalty_weight: 0.15,        // Penalty for leaving gaps
            local_density_radius: 0.5,       // Local density calculation radius
            fill_move_prob: 0.15,            // 15% chance of fill-gap move
        }
    }
}
...
impl EvolvedPacker {
    ...
    /// MORE ANGLES: 12 angles (30 degree granularity) instead of 8
    #[inline]
    fn select_angles(&self, n: usize) -> Vec<f64> {
        // Gen9: 12 angles with 30 degree granularity
        // Order varies based on n % 12 for diversity
        let base = match n % 12 {
            0 => vec![0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0],
            1 => vec![30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0],
            2 => vec![60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0],
            3 => vec![90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0],
            4 => vec![120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0],
            5 => vec![150.0, 180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0],
            6 => vec![180.0, 210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0],
            7 => vec![210.0, 240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0],
            8 => vec![240.0, 270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0],
            9 => vec![270.0, 300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0],
            10 => vec![300.0, 330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0],
            _ => vec![330.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0],
        };
        base
    }
    ...
    /// MORE ANGLES: Use 30 degree rotation increments
    #[inline]
    fn density_aware_move(
        ...
        if is_fill_move {
            ...
            match move_type {
                ...
                2 => {
                    // MORE ANGLES: Rotate with 30 degree increments
            ...
            // Standard boundary-aware moves (from Gen5/Gen6)
            ...
            match move_type {
                ...
                2 => {
                    // MORE ANGLES: Use 30 degree increments
