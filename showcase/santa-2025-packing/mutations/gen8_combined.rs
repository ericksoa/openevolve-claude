//! Evolved Packing Algorithm - Generation 8 COMBINED BEST FEATURES
//!
...
//!
//! MUTATION STRATEGY: COMBINED BEST FEATURES (Gen8)
//! Combine all successful strategies from previous generations:
//!
//! 1. DENSITY from Gen6: Gap detection and density-aware placement
//!    - Grid-based density tracking for efficient gap detection
//!    - Placement scoring rewards filling gaps
//!    - Penalize placements that leave unusable gaps
//!
//! 2. SMART BOUNDARY MOVES from Gen5: 90% boundary focus
//!    - Targeted moves based on which boundary edge tree is blocking
//!    - Edge-aware move operators (slide along boundary, rotate to reduce footprint)
//!    - Gradient descent toward center of bounding box
//!
//! 3. EARLY EXIT from Gen4: Exit when no improvement
//!    - Stop SA when no improvement for threshold iterations
//!    - Faster convergence without wasting computation
//!
//! 4. ASPECT RATIO BALANCING: Prefer square packings
//!    - Strong penalty for unbalanced aspect ratios
//!    - Bias direction selection toward shorter dimension
//!
//! 5. MORE COMPUTE FOR SMALL n: Invest heavily in early trees
//!    - n < 30: 50000 iterations (critical foundation)
//!    - n >= 30: Standard iterations
//!
//! Base: Gen6 champion (94.14 at n=200)
//! Target: Beat Gen6 with combined improvements
...
pub struct EvolvedConfig {
    ...
    pub sa_iterations_small_n: usize,  // More iterations for n < 30
    pub small_n_threshold: usize,       // Threshold for "small n"
    ...
    // Early exit threshold (from Gen4)
    pub early_exit_threshold: usize,

    // Boundary focus probability (from Gen5: 90% boundary, 10% random)
    ...
    // Aspect ratio balancing weight
    pub aspect_ratio_weight: f64,
}

impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen8 COMBINED: Best features from all generations
        Self {
            search_attempts: 300,            // Increased for better placement search
            direction_samples: 80,           // More directions for better coverage
            sa_iterations: 30000,            // Base iterations for large n
            sa_iterations_small_n: 50000,    // NEW: 50000 for n < 30
            small_n_threshold: 30,           // NEW: Threshold for "small n"
            sa_initial_temp: 0.48,           // Slightly lower for exploitation
            sa_cooling_rate: 0.99993,        // Balanced cooling
            sa_min_temp: 0.000005,           // Very low minimum for fine-tuning
            ...
            sa_passes: 2,                    // 2 passes
            early_exit_threshold: 2000,      // From Gen4: exit when stuck
            boundary_focus_prob: 0.90,       // From Gen5: 90% boundary focus
            // DENSITY parameters from Gen6
            density_grid_resolution: 20,     // 20x20 grid for density tracking
            gap_penalty_weight: 0.12,        // Penalty for leaving gaps
            local_density_radius: 0.55,      // Local density calculation radius
            fill_move_prob: 0.12,            // 12% chance of fill-gap move
            // Aspect ratio balancing
            aspect_ratio_weight: 0.14,       // Strong aspect ratio penalty
        }
    }
}
...
impl EvolvedPacker {
    ...
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            // Run SA passes with combined moves
    ...
    /// COMBINED: Density-aware placement with aspect ratio balancing
    fn find_placement(
        &self,
        existing: &[PlacedTree],
        n: usize,
        ...
        // Find gaps in the packing for density-aware placement (from Gen6)
    ...
    /// COMBINED: Density + aspect ratio + limiting dimension penalty
    #[inline]
    fn placement_score(&self, tree: &PlacedTree, existing: &[PlacedTree], n: usize) -> f64 {
        ...
        // ASPECT RATIO BALANCING: Strong penalty for unbalanced bounds
        let balance_penalty = (width - height).abs() * self.config.aspect_ratio_weight;

        // From Gen5: Penalize extending the limiting dimension
        let limiting_dim_penalty = if width > height {
            let tree_extends_x = (tree_max_x > pack_max_x - 0.01) || (tree_min_x < pack_min_x + 0.01);
            if tree_extends_x { 0.04 } else { 0.0 }
        } else {
            let tree_extends_y = (tree_max_y > pack_max_y - 0.01) || (tree_min_y < pack_min_y + 0.01);
            if tree_extends_y { 0.04 } else { 0.0 }
        };

        // DENSITY: Calculate local density around the new tree (from Gen6)
        ...
        side_score + balance_penalty + limiting_dim_penalty + extension_penalty
            + gap_penalty + center_penalty + density_bonus - neighbor_bonus
    }

    /// DENSITY: Calculate local density around a point (from Gen6)
    #[inline]
    ...
    /// DENSITY: Estimate if placement creates an unusable gap (from Gen6)
    #[inline]
    ...
    /// DENSITY: Bonus for being close to existing trees (from Gen6)
    #[inline]
    ...
    /// DENSITY: Find gaps in the current packing (from Gen6)
    ...
    /// COMBINED: Density bias + aspect ratio balancing
    #[inline]
    fn select_direction(&self, n: usize, width: f64, height: f64, rng: &mut impl Rng) -> f64 {
        ...
        if strategy < 0.40 {
            ...
            // ASPECT RATIO BALANCING: Bias toward the shorter dimension
            if width < height {
    ...
    /// COMBINED: Boundary focus + gap-filling + early exit + more compute for small n
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, rng: &mut impl Rng) {
        ...
        // SMALL N BOOST: Use more iterations for n < 30 (critical foundation)
        let base_sa_iters = if n < self.config.small_n_threshold {
        ...
        for iter in 0..base_iterations {
            ...
            // COMBINED: Choose between boundary optimization and gap-filling
            ...
            let (idx, edge) = if do_fill_move {
                ...
                // SMART MOVES from Gen5: 90% boundary focus
                let bi = &boundary_info[rng.gen_range(0..boundary_info.len())];
                (bi.0, bi.1)
            } else {
                (rng.gen_range(0..trees.len()), BoundaryEdge::None)
            ...
            let success = self.combined_move(trees, idx, temp, edge, do_fill_move, rng);
    ...
    /// Find trees on the bounding box boundary (from Gen5)
    #[inline]
    ...
    /// COMBINED: Move operator with density awareness and boundary targeting
    #[inline]
    fn combined_move(
        &self,
        ...
        // DENSITY: If this is a fill move, try to compact the packing
        if is_fill_move {
            let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
            ...
            // SMART MOVES from Gen5: Boundary-aware moves
