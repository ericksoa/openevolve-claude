//! Evolved Packing Algorithm - Generation 9 SA PASSES
//!
...
//!
//! MUTATION STRATEGY: SA PASSES (Gen9)
//! Try 3 SA passes instead of 2 with different temperature schedules:
//!
//! Key changes from Gen6:
//! - sa_passes: 3 (up from 2)
//! - sa_iterations per pass: 20000 (reduced to keep total similar)
//! - Pass 0: High temperature exploration (1.0x temp, fast cooling)
//! - Pass 1: Medium temperature refinement (0.5x temp, medium cooling)
//! - Pass 2: Low temperature exploitation (0.2x temp, slow cooling)
//! - All other parameters kept the same as Gen6 champion
//!
//! Target: Beat Gen6's 94.14 at n=200 with multi-pass SA optimization
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen9 SA PASSES: 3 passes with different temperature schedules
        Self {
            search_attempts: 280,            // Same as Gen6
            direction_samples: 72,           // Same as Gen6
            sa_iterations: 20000,            // Reduced from 28000 (20k * 3 passes ~ 28k * 2 passes)
            sa_initial_temp: 0.45,           // Same as Gen6 (modified per pass)
            sa_cooling_rate: 0.99994,        // Same as Gen6 (modified per pass)
            ...
            sa_passes: 3,                    // CHANGED: 3 passes instead of 2
...
impl EvolvedPacker {
    ...
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            // Run SA passes with different temperature schedules
    ...
    /// GEN9 SA PASSES: Different temperature schedules per pass
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, rng: &mut impl Rng) {
        ...
        // GEN9: Different temperature and cooling schedules per pass
        let (temp_multiplier, cooling_multiplier, iter_multiplier) = match pass {
            0 => {
                // Pass 0: High temperature exploration
                // More exploration, faster cooling
                (1.0, 0.99990, 1.0)
            }
            1 => {
                // Pass 1: Medium temperature refinement
                // Balanced exploration/exploitation
                (0.5, 0.99994, 0.8)
            }
            _ => {
                // Pass 2: Low temperature exploitation
                // Fine-tuning with slow cooling
                (0.2, 0.99997, 0.6)
            }
        };

        let mut temp = self.config.sa_initial_temp * temp_multiplier;
        let cooling_rate = self.config.sa_cooling_rate.powf(cooling_multiplier / self.config.sa_cooling_rate);

        let base_iterations = ((self.config.sa_iterations as f64 * iter_multiplier) as usize) + n * 80;
