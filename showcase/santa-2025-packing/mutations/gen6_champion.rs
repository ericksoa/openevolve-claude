//! Evolved Packing Algorithm - Generation 6 DENSITY MAXIMIZATION
//!
...
//!
//! MUTATION STRATEGY: DENSITY MAXIMIZATION (Gen6)
//! Pack for maximum density rather than just minimum bounding box:
//!
//! Key improvements from Gen5:
//! - Placement scoring: reward placements that fill gaps
//! - Calculate local density around candidate position
//! - Prefer placements that don't leave unusable gaps
//! - Move operators: fill-gap moves that compact interior
//! - Then boundary optimization as normal
//! - Grid-based density tracking for efficient gap detection
//!
//! Target: Beat Gen5's 95.69 at n=200 with density-aware packing
...
pub struct EvolvedConfig {
    ...
    // DENSITY MAXIMIZATION: New parameters
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen6 DENSITY MAXIMIZATION: Density-aware configuration
        Self {
            search_attempts: 280,            // More attempts to find gap-filling placements
            direction_samples: 72,           // More directions for better gap coverage
            sa_iterations: 28000,            // More iterations for density optimization
            sa_initial_temp: 0.45,           // Slightly lower for more exploitation
            sa_cooling_rate: 0.99994,        // Slower cooling for density convergence
            sa_min_temp: 0.000008,           // Lower minimum for fine-tuning
            translation_scale: 0.055,        // Smaller moves for precision gap filling
            rotation_granularity: 45.0,      // Keep 8 angles
            center_pull_strength: 0.07,      // Moderate pull
            sa_passes: 2,                    // Keep 2 passes
            early_exit_threshold: 1800,      // More patience for density moves
