//! Evolved Packing Algorithm - Generation 67a CONTINUOUS ANGLES
//!
//! RADICAL MUTATION: CONTINUOUS ANGLE OPTIMIZATION
//!
//! The fundamental insight from top solutions (70.1 score) is that they use
//! CONTINUOUS angles (21°, 66°, 153°, etc.) rather than discrete 45° steps.
//!
//! This mutation completely redesigns angle handling:
//! 1. During placement: Try many angles (24 steps = 15° each) for broad search
//! 2. During SA: Continuously optimize angles with small perturbations (±1-5°)
//! 3. Post-optimization: Fine-tune angles with gradient-like search
//!
//! Key changes:
//! - 24 angle candidates during placement (15° steps)
//! - Continuous angle mutation in SA (not discrete 45° jumps)
//! - Angle refinement pass after each SA phase
//!
//! Target: Break the 88 barrier by exploiting continuous angle freedom
...
pub struct EvolvedConfig {
    ...
    pub angle_mutation_prob: f64,      // NEW: Probability of angle-only mutation
    pub angle_perturbation_max: f64,   // NEW: Max angle change in SA (degrees)
    pub num_placement_angles: usize,   // NEW: Number of angles to try during placement
}
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            angle_mutation_prob: 0.25,   // NEW: 25% of moves are angle-only
            angle_perturbation_max: 10.0, // NEW: ±10° continuous perturbation
            num_placement_angles: 24,     // NEW: 24 angles (15° steps)
        }
    }
}
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // NEW: Angle refinement pass after SA
                self.refine_angles(&mut trees, &mut rng);
    ...
    /// NEW: Generate continuous angles for placement (not discrete 45° steps)
    fn generate_placement_angles(&self) -> Vec<f64> {
        let n = self.config.num_placement_angles;
        (0..n).map(|i| (i as f64 / n as f64) * 360.0).collect()
    }
    ...
    fn find_placement_with_strategy(
        ...
        if existing.is_empty() {
            // First tree: try a few angles and pick best
            let initial_angle = match strategy {
                ...
                PlacementStrategy::Random => rng.gen_range(0.0..360.0),
        ...
        // NEW: Use continuous angles (24 steps = 15° each)
        let angles = self.generate_placement_angles();
    ...
    /// NEW: Refine angles after SA using gradient-like local search
    fn refine_angles(&self, trees: &mut Vec<PlacedTree>, rng: &mut impl Rng) {
        if trees.len() <= 1 {
            return;
        }

        let mut current_side = compute_side_length(trees);
        let angle_steps = [5.0, 2.0, 1.0, 0.5];  // Decreasing step sizes

        for &step in &angle_steps {
            for idx in 0..trees.len() {
                let old = trees[idx].clone();

                // Try small positive and negative angle changes
                for delta in &[step, -step] {
                    let new_angle = (old.angle_deg + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old.x, old.y, new_angle);
    ...
    fn placement_score(&self, tree: &PlacedTree, existing: &[PlacedTree], n: usize) -> f64 {
        ...
        side_score + balance_penalty + extension_penalty + center_penalty + density_bonus - neighbor_bonus
    }
    ...
    fn local_search(
        ...
        for iter in 0..base_iterations {
            ...
            if move_type_roll < self.config.compression_prob {
                // Compression move
            ...
            } else if move_type_roll < self.config.compression_prob + self.config.angle_mutation_prob {
                // NEW: Continuous angle mutation
                let idx = rng.gen_range(0..trees.len());
                let old = trees[idx].clone();

                // Small continuous angle perturbation
                let angle_delta = rng.gen_range(-self.config.angle_perturbation_max..self.config.angle_perturbation_max);
                let new_angle = (old.angle_deg + angle_delta).rem_euclid(360.0);
                trees[idx] = PlacedTree::new(old.x, old.y, new_angle);
    ...
    fn sa_move(
        ...
        match move_type % 4 {
            ...
            _ => {
                // Combined translation and small angle adjustment
                let dx = rng.gen_range(-scale * 0.5..scale * 0.5);
                let dy = rng.gen_range(-scale * 0.5..scale * 0.5);
                let angle_delta = rng.gen_range(-5.0..5.0);  // Small continuous angle
