//! Evolved Packing Algorithm - Generation 75b FINER 10-DEGREE STEPS
//!
//! MUTATION STRATEGY: USE 10-DEGREE STEPS FOR LATE-STAGE
//! Gen73c showed that late-stage continuous angles help (88.90).
//! Hypothesis: Extending fine angles to 10-degree angle steps instead of 15-degree for
//! n >= 140, giving 36 angles instead of 24.
//!
//! Risk: More trees with fine angles = more computation + potential SA instability
//!
//! Changes from Gen73c:
//! - Change fine_angle_step from 15.0 to 10.0
...
impl EvolvedPacker {
    ...
    fn select_fine_angles_for_strategy(&self, n: usize, strategy: PlacementStrategy) -> Vec<f64> {
        ...
        for &base in &base_angles {
            let plus_10 = (base + 10.0).rem_euclid(360.0);
            let minus_10 = (base - 10.0).rem_euclid(360.0);

            if !fine_angles.contains(&plus_10) {
                fine_angles.push(plus_10);
            }
            if !fine_angles.contains(&minus_10) {
                fine_angles.push(minus_10);
            }
        }

        for &base in &base_angles {
            let plus_20 = (base + 20.0).rem_euclid(360.0);
            let minus_20 = (base - 20.0).rem_euclid(360.0);

            if !fine_angles.contains(&plus_20) {
                fine_angles.push(plus_20);
            }
            if !fine_angles.contains(&minus_20) {
                fine_angles.push(minus_20);
            }
        }
