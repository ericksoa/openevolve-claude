//! Evolved Packing Algorithm - Generation 75c TWO-TIER LATE STAGE
//!
//! MUTATION STRATEGY: TWO-TIER ANGLE GRANULARITY
//! Gen74a showed extending fine angles helps (88.72).
//! Hypothesis: Use progressive refinement:
//! - n >= 140: 15° steps (24 angles) - current behavior
//! - n >= 180: 7.5° steps (48 angles) - even finer for final 10%
//!
//! Risk: Final trees are hardest to place - finer angles might help
//!
//! Changes from Gen74a:
//! - Add very_late_stage_threshold: 180
//! - Add select_very_fine_angles_for_strategy for 7.5° steps
...
pub struct EvolvedConfig {
    ...
    pub very_late_stage_threshold: usize,  // NEW: Second tier for even finer angles
}
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            wave_passes: 3,
            late_stage_threshold: 140,  // Last 30% get 15° steps
            fine_angle_step: 15.0,
            very_late_stage_threshold: 180,  // NEW: Last 10% get 7.5° steps
        }
    }
}
...
impl EvolvedPacker {
    ...
    fn find_placement_with_strategy(
        ...
        // TWO-TIER ANGLE SELECTION:
        // n >= 180: very fine 7.5° steps (48 angles) - final 10%
        // n >= 140: fine 15° steps (24 angles) - last 30%
        // n < 140: standard 45° steps (8 angles)
        let angles = if n >= self.config.very_late_stage_threshold {
            self.select_very_fine_angles_for_strategy(n, strategy)
        } else if n >= self.config.late_stage_threshold {
    ...
    /// Very fine angles for the final 10% of trees (n >= 180)
    /// Uses 7.5° steps for 48 total angles
    #[inline]
    fn select_very_fine_angles_for_strategy(&self, n: usize, strategy: PlacementStrategy) -> Vec<f64> {
        let base_angles = self.select_angles_for_strategy(n, strategy);
        let mut very_fine_angles = Vec::with_capacity(48);

        // Start with base 45° angles
        for &angle in &base_angles {
            very_fine_angles.push(angle);
        }

        // Add 7.5° offsets
        for &base in &base_angles {
            let plus_7_5 = (base + 7.5).rem_euclid(360.0);
            let minus_7_5 = (base - 7.5).rem_euclid(360.0);

            if !very_fine_angles.contains(&plus_7_5) {
                very_fine_angles.push(plus_7_5);
            }
            if !very_fine_angles.contains(&minus_7_5) {
                very_fine_angles.push(minus_7_5);
            }
        }

        // Add 15° offsets (same as regular fine)
        for &base in &base_angles {
            let plus_15 = (base + 15.0).rem_euclid(360.0);
            let minus_15 = (base - 15.0).rem_euclid(360.0);

            if !very_fine_angles.contains(&plus_15) {
                very_fine_angles.push(plus_15);
            }
            if !very_fine_angles.contains(&minus_15) {
                very_fine_angles.push(minus_15);
            }
        }

        // Add 22.5° offsets
        for &base in &base_angles {
            let plus_22_5 = (base + 22.5).rem_euclid(360.0);
            let minus_22_5 = (base - 22.5).rem_euclid(360.0);

            if !very_fine_angles.contains(&plus_22_5) {
                very_fine_angles.push(plus_22_5);
            }
            if !very_fine_angles.contains(&minus_22_5) {
                very_fine_angles.push(minus_22_5);
            }
        }

        // Add 30° offsets (same as regular fine)
        for &base in &base_angles {
            let plus_30 = (base + 30.0).rem_euclid(360.0);
            let minus_30 = (base - 30.0).rem_euclid(360.0);

            if !very_fine_angles.contains(&plus_30) {
                very_fine_angles.push(plus_30);
            }
            if !very_fine_angles.contains(&minus_30) {
                very_fine_angles.push(minus_30);
            }
        }
