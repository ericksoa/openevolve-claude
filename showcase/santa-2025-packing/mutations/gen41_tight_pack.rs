//! Evolved Packing Algorithm - Generation 41 TIGHT PACKING
//!
//! MUTATION STRATEGY: Focus on finding tight placements near existing trees.
//!
//! Changes from Gen28:
//! - For each existing tree, try to place new tree as close as possible
//! - More focused search around contact points
//! - Should create denser packings
...
impl EvolvedPacker {
    ...
    fn find_placement_with_strategy(&self, existing: &[PlacedTree], n: usize, _max_n: usize, strategy: PlacementStrategy, rng: &mut impl Rng) -> PlacedTree {
        ...
        // TIGHT PACKING: For each existing tree, try to place new tree in contact
        for exist in existing {
            let (ex1, ey1, ex2, ey2) = exist.bounds();
            let (ecx, ecy) = ((ex1 + ex2) / 2.0, (ey1 + ey2) / 2.0);

            // Try 16 directions around this tree
            for dir_idx in 0..16 {
                let dir = (dir_idx as f64 / 16.0) * 2.0 * PI;
                let (vx, vy) = (dir.cos(), dir.sin());

                for &tree_angle in &angles {
                    // Binary search to find closest valid position
                    let (mut low, mut high) = (0.0, 3.0);  // Smaller search range
                    while high - low > 0.0005 {  // Higher precision
                        let mid = (low + high) / 2.0;
                        let candidate = PlacedTree::new(ecx + mid * vx, ecy + mid * vy, tree_angle);
                        if is_valid(&candidate, existing) { high = mid; } else { low = mid; }
                    }

                    let candidate = PlacedTree::new(ecx + high * vx, ecy + high * vy, tree_angle);
        ...
        // Also try ray-casting from origin (original approach)
