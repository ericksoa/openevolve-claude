//! Evolved Packing Algorithm - Generation 30C MORE PASSES
//!
//! MUTATION STRATEGY: Add a third SA pass with lower temperature.
//!
//! Changes from Gen28:
//! - sa_passes: 3 (was 2)
//! - Third pass at 0.15 temp multiplier for fine-tuning
//! - Slightly reduced iterations per pass to keep time similar
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            sa_iterations: 20000,         // Reduced per pass (3 passes total)
...
impl EvolvedPacker {
    ...
    fn calculate_local_density(&self, cx: f64, cy: f64, trees: &[PlacedTree]) -> f64 {
        let (radius, radius_sq) = (self.config.local_density_radius, self.config.local_density_radius.powi(2));
        trees.iter().map(|tree| {
    ...
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, _strategy: PlacementStrategy, rng: &mut impl Rng) {
        ...
        // THREE PASS TEMPERATURES
        let temp_multiplier = match pass { 0 => 1.0, 1 => 0.35, _ => 0.15 };
        ...
        let base_iterations = match pass {
            ...
            _ => self.config.sa_iterations / 3 + n * 30,  // Third pass
        };
        ...
        for iter in 0..base_iterations {
            if iterations_without_improvement >= self.config.hot_restart_interval && total_restarts < max_restarts {
                ...
                *trees = elite_pool[elite_idx].1.clone();
                ...
                temp = self.config.hot_restart_temp * (if pass >= 2 { 0.5 } else { 1.0 });
