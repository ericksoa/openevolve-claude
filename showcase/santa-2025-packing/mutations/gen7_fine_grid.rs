//! Evolved Packing Algorithm - Generation 7 FINE GRID
//!
...
//!
//! MUTATION STRATEGY: FINE GRID (Gen7)
//! Gen6 used 20x20 density grid. Try finer resolution for more precise gap detection:
//!
//! Key improvements from Gen6:
//! - density_grid_resolution: 40 (2x finer than Gen6's 20)
//! - More precise gap detection with finer grid cells
//! - Lower local_density_radius for tighter packing (0.35 vs 0.5)
//! - Increased gap_penalty_weight for stronger gap avoidance (0.22 vs 0.15)
//! - Higher fill_move_prob to prioritize gap filling (0.22 vs 0.15)
//!
//! Target: Beat Gen6's 93.23 at n=200 with finer density tracking
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen7 FINE GRID: Finer density grid for more precise gap detection
        Self {
            search_attempts: 300,            // More attempts to find gap-filling placements
            direction_samples: 80,           // More directions for better gap coverage
            sa_iterations: 30000,            // More iterations for fine-grained optimization
            sa_initial_temp: 0.42,           // Slightly lower for more exploitation
            sa_cooling_rate: 0.99995,        // Slower cooling for fine convergence
            sa_min_temp: 0.000006,           // Lower minimum for fine-tuning
            translation_scale: 0.048,        // Smaller moves for precision gap filling
            rotation_granularity: 45.0,      // Keep 8 angles
            center_pull_strength: 0.065,     // Slightly reduced pull
            sa_passes: 2,                    // Keep 2 passes
            early_exit_threshold: 2000,      // More patience for fine-grained moves
            boundary_focus_prob: 0.82,       // 82% boundary, 18% gap-filling
            // FINE GRID parameters - key changes from Gen6
            density_grid_resolution: 40,     // 40x40 grid (2x finer than Gen6's 20x20)
            gap_penalty_weight: 0.22,        // Stronger penalty for leaving gaps (was 0.15)
            local_density_radius: 0.35,      // Tighter radius for local density (was 0.5)
            fill_move_prob: 0.22,            // Higher chance of fill-gap move (was 0.15)
        }
    }
}
...
impl EvolvedPacker {
    ...
    /// FINE GRID: More precise gap detection with finer resolution
    fn find_placement(
        &self,
        existing: &[PlacedTree],
        n: usize,
        ...
        for attempt in 0..self.config.search_attempts {
            // FINE GRID: More frequently target gaps with finer detection
            let dir = if !gaps.is_empty() && attempt % 3 == 0 {
                // Target a gap more frequently (every 3rd attempt vs every 4th)
            ...
            for &tree_angle in &angles {
                ...
                while high - low > 0.0008 {  // Tighter tolerance for finer precision
    ...
    /// FINE GRID: Enhanced gap detection and local density with finer resolution
    #[inline]
    fn placement_score(&self, tree: &PlacedTree, existing: &[PlacedTree], n: usize) -> f64 {
        ...
        let balance_penalty = (width - height).abs() * 0.11;  // Slightly increased

        // FINE GRID: Calculate local density around the new tree with tighter radius
        ...
        // FINE GRID: Penalize placements that extend the bounding box into empty space
        ...
        let extension_penalty = (x_extension + y_extension) * 0.09;  // Slightly increased

        // FINE GRID: Penalize leaving unusable gaps with stronger weight
    ...
    /// FINE GRID: Calculate local density around a point with tighter radius
    #[inline]
    fn calculate_local_density(&self, cx: f64, cy: f64, trees: &[PlacedTree]) -> f64 {
        ...
        for tree in trees {
            ...
            if dist_sq < radius_sq {
                ...
                // FINE GRID: Use squared falloff for tighter density calculation
                let ratio = dist_sq / radius_sq;
                count += 1.0 - ratio;  // Quadratic falloff instead of linear
            }
        }
    ...
    /// FINE GRID: Estimate if placement creates an unusable gap with finer detection
    #[inline]
    fn estimate_unusable_gap(&self, tree: &PlacedTree, existing: &[PlacedTree]) -> f64 {
        ...
        let min_useful_gap = 0.12;  // Slightly smaller for finer detection (was 0.15)
        let max_wasteful_gap = 0.35; // Slightly smaller for finer detection (was 0.4)
    ...
    /// FINE GRID: Bonus for being close to existing trees (promotes compactness)
    #[inline]
    fn neighbor_proximity_bonus(&self, tree: &PlacedTree, existing: &[PlacedTree]) -> f64 {
        ...
        for other in existing {
            ...
            if dist < 0.7 {  // Tighter threshold for finer detection (was 0.8)
                close_neighbors += 1;
            }
        }

        // Bonus for close neighbors and minimum distance
        let dist_bonus = if min_dist < 1.3 { 0.025 * (1.3 - min_dist) } else { 0.0 };  // Tighter
        let neighbor_bonus = 0.006 * close_neighbors as f64;  // Slightly increased

        dist_bonus + neighbor_bonus
    }

    /// FINE GRID: Find gaps in the current packing with 40x40 resolution
    fn find_gaps(&self, trees: &[PlacedTree], min_x: f64, min_y: f64, max_x: f64, max_y: f64) -> Vec<(f64, f64, f64, f64)> {
        ...
        // FINE GRID: Find empty cells that are surrounded by occupied cells (gaps)
        // With 40x40 grid, we can detect smaller gaps more precisely
        for i in 1..grid_res - 1 {
            for j in 1..grid_res - 1 {
                let idx = j * grid_res + i;
                if !occupied[idx] {
                    // Check if surrounded by occupied cells (including diagonals for finer detection)
                    ...
                    // Also check diagonals for more comprehensive gap detection
                    let diag_occupied =
                        occupied[(j - 1) * grid_res + i - 1] as i32 +
                        occupied[(j - 1) * grid_res + i + 1] as i32 +
                        occupied[(j + 1) * grid_res + i - 1] as i32 +
                        occupied[(j + 1) * grid_res + i + 1] as i32;

                    // Consider it a gap if it has at least 2 cardinal neighbors OR 3+ total neighbors
                    if neighbors_occupied >= 2 || (neighbors_occupied + diag_occupied) >= 4 {
                        // This is a gap
    ...
    /// FINE GRID: Bias toward less dense regions to fill gaps
    #[inline]
    fn select_direction(&self, n: usize, width: f64, height: f64, rng: &mut impl Rng) -> f64 {
        ...
        if strategy < 0.42 {
            ...
            base + rng.gen_range(-0.04..0.04)  // Tighter jitter
        } else if strategy < 0.68 {
            // FINE GRID: Bias toward the shorter dimension (to fill before expanding)
            if width < height {
                // Pack more horizontally
                let angle = if rng.gen() { 0.0 } else { PI };
                angle + rng.gen_range(-PI / 4.5..PI / 4.5)  // Tighter range
            } else {
                // Pack more vertically
                let angle = if rng.gen() { PI / 2.0 } else { -PI / 2.0 };
                angle + rng.gen_range(-PI / 4.5..PI / 4.5)  // Tighter range
            }
        } else if strategy < 0.84 {
            // Corner bias
            let corners = [PI / 4.0, 3.0 * PI / 4.0, 5.0 * PI / 4.0, 7.0 * PI / 4.0];
            corners[rng.gen_range(0..4)] + rng.gen_range(-0.12..0.12)  // Tighter jitter
        } else {
            // Golden angle spiral
            let golden_angle = PI * (3.0 - (5.0_f64).sqrt());
            let base = (n as f64 * golden_angle) % (2.0 * PI);
            let offset = rng.gen_range(0..8) as f64 * PI / 4.0;
            (base + offset + rng.gen_range(-0.08..0.08)) % (2.0 * PI)  // Tighter jitter
        }
    }

    /// EVOLVED FUNCTION: Local search with simulated annealing
    /// FINE GRID: Include gap-filling moves with higher probability
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, rng: &mut impl Rng) {
        ...
        let temp_multiplier = match pass {
            0 => 1.0,
            _ => 0.32,  // Slightly lower for finer search
        };
        let mut temp = self.config.sa_initial_temp * temp_multiplier;

        let base_iterations = match pass {
            0 => self.config.sa_iterations + n * 120,  // More iterations
        ...
        for iter in 0..base_iterations {
            ...
            // Update boundary cache every 300 iterations (more frequent for finer search)
            if iter == 0 || iter - boundary_cache_iter >= 300 {
                boundary_info = self.find_boundary_trees_with_edges(trees);
                boundary_cache_iter = iter;
            }

            // FINE GRID: Choose between boundary optimization and gap-filling
            // Higher fill_move_prob means more gap-filling moves
            let do_fill_move = rng.gen::<f64>() < self.config.fill_move_prob;

            let (idx, edge) = if do_fill_move {
                // FINE GRID: Try to fill a gap by moving an interior tree
                let interior_trees: Vec<usize> = (0..trees.len())
                    .filter(|&i| !boundary_info.iter().any(|(bi, _)| *bi == i))
                    .collect();

                if !interior_trees.is_empty() && rng.gen::<f64>() < 0.55 {  // Slightly higher
    ...
    fn find_boundary_trees_with_edges(&self, trees: &[PlacedTree]) -> Vec<(usize, BoundaryEdge)> {
        ...
        let eps = 0.012;  // Tighter epsilon for finer detection
    ...
    /// FINE GRID: Move operator with gap-filling awareness and finer precision
    #[inline]
    fn density_aware_move(
        ...
        let scale = self.config.translation_scale * (0.25 + temp * 1.6);  // Smaller base scale

        // FINE GRID: If this is a fill move, try to move toward less dense areas within bbox
        if is_fill_move {
            let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
            ...
            let move_type = rng.gen_range(0..5);  // Added one more move type
            match move_type {
                0 => {
                    // Move toward center of bbox (compacting)
                    let dx = (bbox_cx - old_x) * 0.08 * (0.4 + temp);  // Smaller moves
                    let dy = (bbox_cy - old_y) * 0.08 * (0.4 + temp);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                1 => {
                    // Small random move to fill gaps
                    let dx = rng.gen_range(-scale * 0.35..scale * 0.35);  // Smaller
                    let dy = rng.gen_range(-scale * 0.35..scale * 0.35);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                2 => {
                    // Rotate to potentially fill gap better
                    let angles = [45.0, 90.0, -45.0, -90.0, 30.0, -30.0, 15.0, -15.0];  // Finer rotations
                ...
                3 => {
                    ...
                    if !gaps.is_empty() {
                        ...
                        let gap_cx = (gap.0 + gap.2) / 2.0;
                        let gap_cy = (gap.1 + gap.3) / 2.0;
                        let dx = (gap_cx - old_x) * 0.04;  // Smaller step
                        let dy = (gap_cy - old_y) * 0.04;
                        trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                    } else {
                        return false;
                    }
                ...
                _ => {
                    // FINE GRID: New move - micro-adjustment for fine packing
            ...
            match move_type {
                0 => {
                    let dx = rng.gen_range(scale * 0.25..scale * 0.9);  // Slightly smaller
                    let dy = rng.gen_range(-scale * 0.18..scale * 0.18);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                1 => {
                    let dy = rng.gen_range(-scale * 0.9..scale * 0.9);
                ...
                3 => {
                    let dx = rng.gen_range(-scale * 0.45..scale * 0.45);  // Slightly smaller
                    let dy = rng.gen_range(-scale * 0.45..scale * 0.45);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                4 => {
                    let dx = rng.gen_range(-scale * 0.9..-scale * 0.25);
                    let dy = rng.gen_range(-scale * 0.18..scale * 0.18);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                5 => {
                    let dx = rng.gen_range(-scale * 0.18..scale * 0.18);
                    let dy = rng.gen_range(-scale * 0.9..-scale * 0.25);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                6 => {
                    let dx = rng.gen_range(-scale * 0.9..scale * 0.9);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y, old_angle);
                }
                7 => {
                    let dx = rng.gen_range(-scale * 0.18..scale * 0.18);
                    let dy = rng.gen_range(scale * 0.25..scale * 0.9);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                8 => {
                    let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
                    ...
                    let dx = (bbox_cx - old_x) * self.config.center_pull_strength * (0.4 + temp);
                    let dy = (bbox_cy - old_y) * self.config.center_pull_strength * (0.4 + temp);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                9 => {
                    let diag = rng.gen_range(-scale * 0.9..scale * 0.9);
