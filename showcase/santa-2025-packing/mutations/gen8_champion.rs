//! Evolved Packing Algorithm - Generation 8 MICRO-TRANSLATIONS
//!
...
//!
//! MUTATION STRATEGY: MICRO-TRANSLATIONS (Gen8)
//! Focus on very precise tree positioning for fine-grained optimization:
//!
//! Key improvements from Gen6:
//! - translation_scale: 0.02 (much smaller than Gen6's 0.055)
//! - Binary search precision: 0.00001 (10x more precise)
//! - More SA iterations: 60000 (to explore fine-grained space)
//! - Fine rotation moves: 5-degree increments
//! - Very slow cooling: 0.999995 (for thorough exploration)
//!
//! Target: Beat Gen6's 94.14 at n=200 with micro-precise positioning
...
pub struct EvolvedConfig {
    ...
    // DENSITY MAXIMIZATION: Parameters inherited from Gen6
    pub density_grid_resolution: usize,
    pub gap_penalty_weight: f64,
    pub local_density_radius: f64,
    pub fill_move_prob: f64,

    // MICRO-TRANSLATIONS: New parameters for Gen8
    pub binary_search_precision: f64,      // Much finer binary search
    pub fine_rotation_increment: f64,      // 5-degree fine rotations
}

impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen8 MICRO-TRANSLATIONS: Very precise positioning configuration
        Self {
            search_attempts: 280,            // Keep from Gen6
            direction_samples: 72,           // Keep from Gen6
            sa_iterations: 60000,            // MICRO: More iterations for fine-grained exploration
            sa_initial_temp: 0.45,           // Keep from Gen6
            sa_cooling_rate: 0.999995,       // MICRO: Very slow cooling for thorough exploration
            sa_min_temp: 0.0000001,          // MICRO: Much lower minimum for ultra-fine tuning
            translation_scale: 0.02,         // MICRO: Much smaller moves (was 0.055)
            rotation_granularity: 45.0,      // Keep 8 angles for initial placement
            center_pull_strength: 0.07,      // Keep from Gen6
            sa_passes: 2,                    // Keep 2 passes
            early_exit_threshold: 3500,      // MICRO: More patience for micro-moves
            ...
            fill_move_prob: 0.15,
            // MICRO-TRANSLATIONS: New parameters
            binary_search_precision: 0.00001, // MICRO: 10x more precise than Gen6
            fine_rotation_increment: 5.0,     // MICRO: Fine 5-degree rotations
        }
    }
}
...
impl EvolvedPacker {
    ...
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            // Run SA passes with micro-translation moves
    ...
    /// MICRO: Use much finer binary search precision
    fn find_placement(
        &self,
        existing: &[PlacedTree],
        n: usize,
        ...
        for attempt in 0..self.config.search_attempts {
            ...
            for &tree_angle in &angles {
                // MICRO: Binary search with much finer precision (0.00001 vs 0.001)
                let mut low = 0.0;
                let mut high = 12.0;
    ...
    fn estimate_unusable_gap(&self, tree: &PlacedTree, existing: &[PlacedTree]) -> f64 {
        ...
        let _tree_width = tree_max_x - tree_min_x;
        let _tree_height = tree_max_y - tree_min_y;
    ...
    /// MICRO: Include fine 5-degree increments for precise positioning
    #[inline]
    fn select_angles(&self, n: usize) -> Vec<f64> {
        // Base angles (8 directions)
        let base = match n % 4 {
        ...
        // MICRO: Add fine 5-degree increments around the primary angles
        let mut angles = base.clone();
        let fine_inc = self.config.fine_rotation_increment;
        for &angle in &base[..4] {  // Add fine increments around first 4 angles
            angles.push((angle + fine_inc).rem_euclid(360.0));
            angles.push((angle - fine_inc).rem_euclid(360.0));
            angles.push((angle + 2.0 * fine_inc).rem_euclid(360.0));
            angles.push((angle - 2.0 * fine_inc).rem_euclid(360.0));
        }
    ...
    /// MICRO: More iterations with very slow cooling for fine-grained exploration
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, rng: &mut impl Rng) {
        ...
        // MICRO: More iterations for thorough exploration
        let base_iterations = match pass {
            0 => self.config.sa_iterations + n * 150,  // More iterations per tree
        ...
        for iter in 0..base_iterations {
            ...
            let success = self.micro_translation_move(trees, idx, temp, edge, do_fill_move, rng);
            ...
            // MICRO: Very slow cooling (0.999995)
    ...
    /// MICRO: Move operator with micro-translation precision
    /// Uses very small moves and fine rotation increments
    #[inline]
    fn micro_translation_move(
        &self,
        ...
        // MICRO: Much smaller translation scale (0.02 vs 0.055)
        let scale = self.config.translation_scale * (0.3 + temp * 1.5);
        let fine_inc = self.config.fine_rotation_increment;

        // MICRO: If this is a fill move, try to move toward less dense areas within bbox
        if is_fill_move {
            let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
            ...
            match move_type {
                ...
                1 => {
                    // MICRO: Very small random move to fill gaps
                    let dx = rng.gen_range(-scale * 0.3..scale * 0.3);
                    let dy = rng.gen_range(-scale * 0.3..scale * 0.3);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                2 => {
                    // MICRO: Fine rotation (5-degree increments)
                    let delta = if rng.gen() { fine_inc } else { -fine_inc };
                    let new_angle = (old_angle + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old_x, old_y, new_angle);
                }
                3 => {
                    // MICRO: Very fine rotation (2.5-degree increments)
                    let delta = if rng.gen() { fine_inc / 2.0 } else { -fine_inc / 2.0 };
                    let new_angle = (old_angle + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old_x, old_y, new_angle);
                }
                _ => {
                    ...
                    if !gaps.is_empty() {
                        ...
                        let gap_cx = (gap.0 + gap.2) / 2.0;
                        let gap_cy = (gap.1 + gap.3) / 2.0;
                        let dx = (gap_cx - old_x) * 0.03;  // MICRO: Smaller step
                        let dy = (gap_cy - old_y) * 0.03;
                        trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                    } else {
                        return false;
                    }
            ...
            // MICRO: Standard boundary-aware moves with smaller translations
            ...
            match move_type {
                0 => {
                    // Move right (smaller scale)
                    let dx = rng.gen_range(scale * 0.2..scale * 0.8);
                    let dy = rng.gen_range(-scale * 0.15..scale * 0.15);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                1 => {
                    // Vertical only (smaller)
                    let dy = rng.gen_range(-scale * 0.8..scale * 0.8);
                    trees[idx] = PlacedTree::new(old_x, old_y + dy, old_angle);
                }
                2 => {
                    // MICRO: Fine rotations (5-degree increments instead of 45)
                    let angles = [fine_inc, -fine_inc, 2.0 * fine_inc, -2.0 * fine_inc,
                                  3.0 * fine_inc, -3.0 * fine_inc];
                    let delta = angles[rng.gen_range(0..angles.len())];
                    let new_angle = (old_angle + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old_x, old_y, new_angle);
                }
                3 => {
                    // MICRO: Very small random move
                    let dx = rng.gen_range(-scale * 0.4..scale * 0.4);
                    let dy = rng.gen_range(-scale * 0.4..scale * 0.4);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                4 => {
                    // Move left (smaller scale)
                    let dx = rng.gen_range(-scale * 0.8..-scale * 0.2);
                    let dy = rng.gen_range(-scale * 0.15..scale * 0.15);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                5 => {
                    // Move down (smaller scale)
                    let dx = rng.gen_range(-scale * 0.15..scale * 0.15);
                    let dy = rng.gen_range(-scale * 0.8..-scale * 0.2);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                6 => {
                    // Horizontal only (smaller)
                    let dx = rng.gen_range(-scale * 0.8..scale * 0.8);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y, old_angle);
                }
                7 => {
                    // Move up (smaller scale)
                    let dx = rng.gen_range(-scale * 0.15..scale * 0.15);
                    let dy = rng.gen_range(scale * 0.2..scale * 0.8);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                8 => {
                    // Center pull (smaller)
                    ...
                    let dx = (bbox_cx - old_x) * self.config.center_pull_strength * 0.5 * (0.5 + temp);
                    let dy = (bbox_cy - old_y) * self.config.center_pull_strength * 0.5 * (0.5 + temp);
                    trees[idx] = PlacedTree::new(old_x + dx, old_y + dy, old_angle);
                }
                9 => {
                    // Diagonal (smaller)
                    let diag = rng.gen_range(-scale * 0.6..scale * 0.6);
                    let sign = if rng.gen() { 1.0 } else { -1.0 };
                    trees[idx] = PlacedTree::new(old_x + diag, old_y + sign * diag, old_angle);
                }
                10 => {
                    // MICRO: Extra fine rotation (2.5-degree or 1-degree)
                    let very_fine = [fine_inc / 2.0, -fine_inc / 2.0, fine_inc / 5.0, -fine_inc / 5.0];
                    let delta = very_fine[rng.gen_range(0..very_fine.len())];
                    let new_angle = (old_angle + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old_x, old_y, new_angle);
                }
                _ => {
                    ...
                    if mag > 0.08 {
                        let delta_r = rng.gen_range(-0.03..0.03) * (1.0 + temp);  // Smaller radial
