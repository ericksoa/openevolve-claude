//! Evolved Packing Algorithm - Generation 60 SWAP MOVES
//!
//! MUTATION STRATEGY: SWAP MOVES IN SA
//! Add tree position swapping as an SA move to allow more aggressive exploration.
//!
//! Key insight: Sometimes two trees would pack better if they swapped positions.
//! This adds swap moves to the SA neighborhood.
//!
//! Changes from Gen47:
//! - Add swap_move function that swaps positions of two trees
//! - 15% probability of swap move during SA
//!
//! Target: Better escape from local optima
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            elite_pool_size: 3,
            swap_move_prob: 0.15,  // NEW: 15% chance of swap move
        }
    }
}
...
impl EvolvedPacker {
    ...
    fn local_search(
        ...
        for iter in 0..base_iterations {
            ...
            // NEW: Check if we should do a swap move
            let do_swap = rng.gen::<f64>() < self.config.swap_move_prob && trees.len() >= 2;

            if do_swap {
                // Try swapping two trees' positions
    ...
    // NEW: Swap move - swap positions of two trees
    fn swap_move(&self, trees: &mut [PlacedTree], rng: &mut impl Rng) -> bool {
        ...
        let mut idx2 = rng.gen_range(0..trees.len());
        while idx2 == idx1 {
            idx2 = rng.gen_range(0..trees.len());
        }

        // Get positions (keep angles the same)
        let x1 = trees[idx1].x;
        let y1 = trees[idx1].y;
