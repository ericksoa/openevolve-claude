//! Evolved Packing Algorithm - Generation 38 ADAPTIVE TEMPERATURE
//!
//! MUTATION STRATEGY: Adaptive temperature control based on acceptance rate.
//!
//! Changes from Gen28:
//! - Track acceptance rate over last 100 moves
//! - If acceptance rate < 20%, increase temperature (heat up)
//! - If acceptance rate > 50%, decrease temperature (cool down)
//! - More aggressive moves at higher temperatures
...
impl EvolvedPacker {
    ...
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, _strategy: PlacementStrategy, rng: &mut impl Rng) {
        ...
        for iter in 0..base_iterations {
            ...
            } else { trees[idx] = old_tree; iterations_without_improvement += 1; recent_total += 1; }

            // Adaptive temperature adjustment every 100 iterations
            if recent_total >= self.config.acceptance_window {
                let acceptance_rate = recent_accepts as f64 / recent_total as f64;
                if acceptance_rate < 0.15 {
                    // Heat up if too cold
                    temp = (temp * 1.05).min(self.config.sa_initial_temp);
                } else if acceptance_rate > 0.5 {
                    // Cool down faster if too hot
                    temp *= 0.95;
                }
                recent_accepts = 0;
                recent_total = 0;
