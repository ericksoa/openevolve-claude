//! Evolved Packing Algorithm - Generation 46 DIAGONAL PREFERENCE
//!
//! MUTATION STRATEGY: DIAGONAL ANGLE PREFERENCE
//! Prioritize 45-degree angles for tree placement.
//!
//! Key insight: Trees at 45-degree angles form diamond shapes which
//! might interlock better with each other, reducing wasted space.
//!
//! Changes from Gen28:
//! - Primary angles are 45, 135, 225, 315 (diagonal)
//! - Cardinal angles (0, 90, 180, 270) are secondary
//! - Diagonal direction preference in placement search
//!
//! Target: Better interlocking through diagonal orientation
...
impl EvolvedPacker {
    ...
    fn find_placement_with_strategy(
        ...
        // DIAGONAL PREFERENCE: prioritize diagonal angles
    ...
    fn select_angles_for_strategy(&self, n: usize, strategy: PlacementStrategy) -> Vec<f64> {
        // DIAGONAL PREFERENCE: Try diagonal angles first
        match strategy {
            PlacementStrategy::ClockwiseSpiral => {
            ...
            PlacementStrategy::Grid => {
                // Alternate based on n for variety
                if n % 2 == 0 {
                    vec![45.0, 135.0, 225.0, 315.0, 0.0, 90.0, 180.0, 270.0]
                } else {
            ...
            PlacementStrategy::BoundaryFirst => {
                // Diagonals are better for corners
                vec![45.0, 135.0, 225.0, 315.0, 0.0, 90.0, 180.0, 270.0]
            }
        }
    }
    ...
    fn select_direction_for_strategy(
        ...
        match strategy {
            ...
            PlacementStrategy::Grid => {
                // DIAGONAL PREFERENCE: bias toward diagonal directions
                let num_dirs = 16;
                let base_idx = attempt % num_dirs;
                let base = (base_idx as f64 / num_dirs as f64) * 2.0 * PI;
                // Slight bias toward diagonal directions
                if attempt % 4 == 0 {
            ...
            PlacementStrategy::BoundaryFirst => {
                ...
                if prob < 0.5 {
                    // Strongly prefer diagonal corners
    ...
    fn sa_move(
        ...
        if is_fill_move {
            ...
            match move_type {
                ...
                2 => {
                    // DIAGONAL PREFERENCE: prefer diagonal rotations
            ...
            match move_type {
                ...
                2 => {
                    // DIAGONAL PREFERENCE: prefer 45-degree rotations
                    let angles = [45.0, -45.0, 90.0, -90.0];
                    let delta = angles[rng.gen_range(0..angles.len())];
                    let new_angle = (old_angle + delta).rem_euclid(360.0);
                    trees[idx] = PlacedTree::new(old_x, old_y, new_angle);
