//! Evolved Packing Algorithm - Generation 36 HEXAGONAL PACKING
//!
//! MUTATION STRATEGY: Add hexagonal packing placement strategy.
//!
//! Changes from Gen28:
//! - Add HexagonalPack as a new placement strategy
//! - Hexagonal packing is theoretically optimal for circles
//! - Try to place trees in a honeycomb pattern
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            num_strategies: 6,  // 6 strategies now
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        let strategies = [
            ...
            PlacementStrategy::HexagonalPack,  // NEW
        ];
    ...
    fn find_placement_with_strategy(&self, existing: &[PlacedTree], n: usize, _max_n: usize, strategy: PlacementStrategy, rng: &mut impl Rng) -> PlacedTree {
        if existing.is_empty() {
            let initial_angle = match strategy {
                ...
                PlacementStrategy::HexagonalPack => 0.0,
            };
        ...
        // For hexagonal packing, try hexagonal grid positions
        if matches!(strategy, PlacementStrategy::HexagonalPack) {
            let hex_spacing = 0.25;  // Approximate tree diameter
            let hex_height = hex_spacing * (3.0_f64).sqrt() / 2.0;

            // Generate hexagonal grid positions around current bounds
            let expand = 1.0;
            for row in -20..=20 {
                for col in -20..=20 {
                    let x = col as f64 * hex_spacing + if row % 2 == 1 { hex_spacing / 2.0 } else { 0.0 };
                    let y = row as f64 * hex_height;

                    // Only try positions near the current bounding box
                    if x < min_x - expand || x > max_x + expand ||
                       y < min_y - expand || y > max_y + expand {
                        continue;
                    }
    ...
    fn select_angles_for_strategy(&self, n: usize, strategy: PlacementStrategy) -> Vec<f64> {
        match strategy {
            ...
            PlacementStrategy::HexagonalPack => vec![0.0, 60.0, 120.0, 180.0, 240.0, 300.0],  // 6-fold symmetry
        }
    }
    ...
    fn select_direction_for_strategy(&self, n: usize, width: f64, height: f64, strategy: PlacementStrategy, attempt: usize, rng: &mut impl Rng) -> f64 {
        match strategy {
            ...
            PlacementStrategy::HexagonalPack => {
                // 6 main directions for hexagonal packing
                let hex_dirs = [0.0, PI/3.0, 2.0*PI/3.0, PI, 4.0*PI/3.0, 5.0*PI/3.0];
                hex_dirs[attempt % 6] + rng.gen_range(-0.05..0.05)
            }
        }
    }
