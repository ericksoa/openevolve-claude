//! Evolved Packing Algorithm - Generation 67d HYBRID CROSSOVER
//!
//! RADICAL MUTATION: CROSSOVER OF GEN47 + GEN62 + NEW IDEAS
//!
//! Combines the best innovations from:
//! - Gen47 ConcentricRings (first sub-90 breakthrough)
//! - Gen62 Radius compression (current champion 88.22)
//! - NEW: Continuous angles during SA (from 67a concept)
//! - NEW: Global rotation optimization (from 67c concept)
//! - NEW: Tighter tangent-seeking moves
//!
//! This is a "kitchen sink" approach combining all known improvements
//! into one aggressive hybrid.
//!
//! Target: Break 87 by combining all proven techniques
...
pub struct EvolvedConfig {
    ...
    pub angle_mutation_prob: f64,
    pub global_rotation_prob: f64,
    pub tangent_move_prob: f64,  // NEW: Try to move closer to neighbors
}
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            search_attempts: 200,
            sa_iterations: 30000,  // Slightly more iterations
            sa_initial_temp: 0.50,  // Slightly higher temp
            sa_cooling_rate: 0.99993,
            sa_min_temp: 0.00001,
            translation_scale: 0.055,
            center_pull_strength: 0.08,  // Slightly stronger pull
            sa_passes: 2,
            early_exit_threshold: 2500,
            boundary_focus_prob: 0.80,
            hot_restart_interval: 800,
            hot_restart_temp: 0.35,
            elite_pool_size: 4,  // Keep more elite solutions
            compression_prob: 0.18,
            angle_mutation_prob: 0.12,  // 12% angle-only moves
            global_rotation_prob: 0.05, // 5% global rotation
            tangent_move_prob: 0.10,    // 10% tangent-seeking moves
        }
    }
}
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // Post-SA optimizations
                self.refine_angles(&mut trees);
    ...
    fn refine_angles(&self, trees: &mut Vec<PlacedTree>) {
        if trees.len() <= 1 {
            return;
        }

        let mut current_side = compute_side_length(trees);
        let angle_steps = [3.0, 1.5, 0.5];
    ...
    fn optimize_global_rotation(&self, trees: &mut Vec<PlacedTree>) {
        ...
        let mut current_side = compute_side_length(trees);
        let angles_to_try = [0.5, 1.0, 1.5, 2.0, -0.5, -1.0, -1.5, -2.0];
    ...
    fn find_placement_with_strategy(
        ...
        // Use 12 angles (30° steps) for more coverage than 8
        let angles: Vec<f64> = (0..12).map(|i| i as f64 * 30.0).collect();
    ...
    fn placement_score(&self, tree: &PlacedTree, existing: &[PlacedTree], n: usize) -> f64 {
        ...
        let neighbor_bonus = if min_neighbor_dist < 1.2 { 0.03 * (1.2 - min_neighbor_dist) } else { 0.0 };
    ...
    fn local_search(
        ...
        for iter in 0..base_iterations {
            ...
            if move_roll < self.config.compression_prob {
                // Compression move (from Gen62)
            ...
            } else if move_roll < self.config.compression_prob + self.config.angle_mutation_prob {
                // Continuous angle mutation
                let idx = rng.gen_range(0..trees.len());
                let old = trees[idx].clone();

                let angle_delta = rng.gen_range(-8.0..8.0);
            ...
            } else if move_roll < self.config.compression_prob + self.config.angle_mutation_prob + self.config.global_rotation_prob {
                // Global rotation
                let old_trees = trees.clone();
            ...
            } else if move_roll < self.config.compression_prob + self.config.angle_mutation_prob + self.config.global_rotation_prob + self.config.tangent_move_prob {
                // Tangent-seeking move: try to get closer to a neighbor
                let idx = rng.gen_range(0..trees.len());
                let old = trees[idx].clone();
                ...
                for (i, other) in trees.iter().enumerate() {
                    if i != idx {
                        let dx = other.x - old.x;
                        let dy = other.y - old.y;
                ...
                // Move slightly toward neighbor
                let neighbor = &trees[nearest_idx];
                let dx = neighbor.x - old.x;
                let dy = neighbor.y - old.y;
                let move_factor = rng.gen_range(0.02..0.06);
                let new_x = old.x + dx * move_factor;
                let new_y = old.y + dy * move_factor;
