//! Evolved Packing Algorithm - Generation 31A MORE ANGLES
//!
//! MUTATION STRATEGY: Try more rotation angles during initial placement.
//!
//! Changes from Gen28:
//! - Added 15-degree and 22.5-degree angles to search
//! - More angle variations in placement strategies
//! - This should help find tighter initial placements
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            rotation_granularity: 22.5,  // Finer rotation (was 45.0)
...
impl EvolvedPacker {
    ...
    fn find_placement_with_strategy(&self, existing: &[PlacedTree], n: usize, _max_n: usize, strategy: PlacementStrategy, rng: &mut impl Rng) -> PlacedTree {
        if existing.is_empty() {
            let initial_angle = match strategy {
                ...
                PlacementStrategy::Random => rng.gen_range(0..16) as f64 * 22.5,  // 16 angles
    ...
    // MORE ANGLES - 16 instead of 8
    fn select_angles_for_strategy(&self, n: usize, strategy: PlacementStrategy) -> Vec<f64> {
        match strategy {
            PlacementStrategy::ClockwiseSpiral => vec![0.0, 22.5, 45.0, 67.5, 90.0, 112.5, 135.0, 157.5, 180.0, 202.5, 225.0, 247.5, 270.0, 292.5, 315.0, 337.5],
            PlacementStrategy::CounterclockwiseSpiral => vec![337.5, 315.0, 292.5, 270.0, 247.5, 225.0, 202.5, 180.0, 157.5, 135.0, 112.5, 90.0, 67.5, 45.0, 22.5, 0.0],
            PlacementStrategy::Grid => vec![0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0, 22.5, 67.5, 112.5, 157.5, 202.5, 247.5, 292.5, 337.5],
            PlacementStrategy::Random => {
                match n % 4 {
                    0 => vec![0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0, 22.5, 67.5, 112.5, 157.5, 202.5, 247.5, 292.5, 337.5],
                    1 => vec![90.0, 270.0, 0.0, 180.0, 45.0, 135.0, 225.0, 315.0, 22.5, 112.5, 202.5, 292.5, 67.5, 157.5, 247.5, 337.5],
                    2 => vec![180.0, 0.0, 270.0, 90.0, 225.0, 45.0, 315.0, 135.0, 22.5, 67.5, 112.5, 157.5, 202.5, 247.5, 292.5, 337.5],
                    _ => vec![270.0, 90.0, 180.0, 0.0, 315.0, 135.0, 225.0, 45.0, 292.5, 112.5, 202.5, 22.5, 337.5, 157.5, 247.5, 67.5],
                }
            }
            PlacementStrategy::BoundaryFirst => vec![45.0, 135.0, 225.0, 315.0, 0.0, 90.0, 180.0, 270.0, 22.5, 67.5, 112.5, 157.5, 202.5, 247.5, 292.5, 337.5],
        }
    }
    ...
    fn sa_move(&self, trees: &mut [PlacedTree], idx: usize, temp: f64, edge: BoundaryEdge, is_fill_move: bool, rng: &mut impl Rng) -> bool {
        ...
        if is_fill_move {
            ...
            match rng.gen_range(0..4) {
                ...
                2 => { let angles = [22.5, 45.0, 67.5, 90.0, -22.5, -45.0]; trees[idx] = PlacedTree::new(old_x, old_y, (old_angle + angles[rng.gen_range(0..6)]).rem_euclid(360.0)); }
                _ => {
            ...
            match move_type {
                ...
                2 => { let angles = [22.5, 45.0, 67.5, 90.0, -22.5, -45.0, -67.5, -90.0]; trees[idx] = PlacedTree::new(old_x, old_y, (old_angle + angles[rng.gen_range(0..8)]).rem_euclid(360.0)); }
