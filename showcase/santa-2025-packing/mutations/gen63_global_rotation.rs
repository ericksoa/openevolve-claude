//! Evolved Packing Algorithm - Generation 63 GLOBAL ROTATION
//!
//! MUTATION STRATEGY: GLOBAL ROTATION OPTIMIZATION
//! After placing trees, try rotating the entire packing by small angles
//! to find an orientation that minimizes the bounding box.
//!
//! Key insight: Top solutions use global rotation optimization (3° steps).
//! Rotating the entire packing can reduce bounding box without changing
//! relative tree positions.
//!
//! Changes from Gen62:
//! - Add global_rotation_optimize() after SA passes
//! - Try rotations from -15° to +15° in 3° steps
//! - Accept rotation that minimizes side length
//!
//! Target: Reduce bounding box via global rotation
...
pub struct EvolvedConfig {
    ...
    pub global_rotation_range: f64,  // NEW: Max rotation to try (+/-)
}
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            global_rotation_step: 3.0,   // NEW: 3° steps like top solutions
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // NEW: Apply global rotation optimization after SA
                self.global_rotation_optimize(&mut trees);
    ...
    fn global_rotation_optimize(&self, trees: &mut Vec<PlacedTree>) {
        if trees.len() <= 1 {
            return;
        }
        ...
        // Compute center of current packing
        let (min_x, min_y, max_x, max_y) = compute_bounds(trees);
        let center_x = (min_x + max_x) / 2.0;
        let center_y = (min_y + max_y) / 2.0;

        // Try rotations from -range to +range in step increments
        let mut angle = -self.config.global_rotation_range;
        while angle <= self.config.global_rotation_range {
            if angle.abs() < 0.01 {
                angle += self.config.global_rotation_step;
                continue;
            }
            ...
            // Compute side length for rotated packing
            ...
            for tree in trees.iter() {
                ...
                let new_angle = tree.angle_deg + angle;

                // Create rotated tree and get bounds
                let rotated = PlacedTree::new(new_x, new_y, new_angle);
                let (bx1, by1, bx2, by2) = rotated.bounds();
        ...
        if best_angle.abs() > 0.01 && best_side < current_side {
            let angle_rad = best_angle * PI / 180.0;
            let cos_a = angle_rad.cos();
            let sin_a = angle_rad.sin();
            ...
            for i in 0..trees.len() {
                ...
                let new_x = center_x + dx * cos_a - dy * sin_a;
                let new_y = center_y + dx * sin_a + dy * cos_a;
                let new_angle = trees[i].angle_deg + best_angle;
