//! Evolved Packing Algorithm - Generation 10 WEIGHT BY N
//!
...
//!
//! MUTATION STRATEGY: WEIGHT BY N (Gen10)
//! Competition score is sum(side²/n). Small n matters disproportionately:
//!
//! Key insight from scoring formula:
//! - n=1 contributes side²/1 while n=200 contributes side²/200
//! - Score weight for n: 1/n (n=1 is 200x more important than n=200)
//! - Allocate computation proportionally to score impact
//!
//! Computational allocation:
//! - For n=1-10: 100000 SA iterations, 500 search attempts (critical)
//! - For n=11-50: 50000 SA iterations, 350 search attempts (important)
//! - For n=51-100: 25000 SA iterations, 250 search attempts (moderate)
//! - For n=101-150: 15000 SA iterations, 180 search attempts (lesser)
//! - For n>150: 5000 SA iterations, 100 search attempts (minimal)
//!
//! Base: Gen6 DENSITY MAXIMIZATION champion (94.14 at n=200)
//! Target: Beat Gen6's 94.14 with score-weighted resource allocation
...
pub struct EvolvedConfig {
    // Search parameters - now dynamically computed based on n
    pub base_search_attempts: usize,
    pub direction_samples: usize,

    // Simulated annealing - base values, scaled by n
    ...
    // Early exit threshold - scaled by n importance
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        // Gen10 WEIGHT BY N: Base configuration (scaled dynamically)
        Self {
            base_search_attempts: 280,    // Base value, scaled by n
            direction_samples: 72,        // More directions for better coverage
            base_sa_iterations: 28000,    // Base value, scaled by n
            sa_initial_temp: 0.45,        // Slightly lower for more exploitation
            sa_cooling_rate: 0.99994,     // Slower cooling for convergence
            sa_min_temp: 0.000008,        // Lower minimum for fine-tuning
            translation_scale: 0.055,     // Smaller moves for precision
            rotation_granularity: 45.0,   // Keep 8 angles
            center_pull_strength: 0.07,   // Moderate pull
            sa_passes: 2,                 // Keep 2 passes
            base_early_exit_threshold: 1800,  // Base patience
            boundary_focus_prob: 0.85,    // 85% boundary, 15% gap-filling
...
impl EvolvedPacker {
    /// WEIGHT BY N: Get scaled search attempts based on n importance
    #[inline]
    fn get_search_attempts(&self, n: usize) -> usize {
        match n {
            1..=10 => 500,      // Critical: 200x-20x score weight
            11..=50 => 350,     // Important: 20x-4x score weight
            51..=100 => 250,    // Moderate: 4x-2x score weight
            101..=150 => 180,   // Lesser: 2x-1.3x score weight
            _ => 100,           // Minimal: <1.3x score weight
        }
    }

    /// WEIGHT BY N: Get scaled SA iterations based on n importance
    #[inline]
    fn get_sa_iterations(&self, n: usize) -> usize {
        match n {
            1..=10 => 100000,   // Critical: maximum effort
            11..=50 => 50000,   // Important: high effort
            51..=100 => 25000,  // Moderate: medium effort
            101..=150 => 15000, // Lesser: reduced effort
            _ => 5000,          // Minimal: basic effort
        }
    }

    /// WEIGHT BY N: Get scaled early exit threshold
    #[inline]
    fn get_early_exit_threshold(&self, n: usize) -> usize {
        match n {
            1..=10 => 8000,     // Very patient for critical n
            11..=50 => 4000,    // Patient for important n
            51..=100 => 2500,   // Moderate patience
            101..=150 => 1500,  // Less patient
            _ => 800,           // Quick exit for unimportant n
        }
    }
    ...
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            // Run SA passes with WEIGHTED iteration counts
    ...
    /// WEIGHT BY N: Use scaled search attempts based on n importance
    fn find_placement(
        &self,
        existing: &[PlacedTree],
        n: usize,
        ...
        // WEIGHT BY N: Scale search attempts by n importance
    ...
    fn select_direction(&self, n: usize, width: f64, height: f64, rng: &mut impl Rng) -> f64 {
        ...
        if strategy < 0.45 {
            ...
            // DENSITY: Bias toward the shorter dimension
            if width < height {
    ...
    /// WEIGHT BY N: Scale iterations and patience by n importance
    fn local_search(&self, trees: &mut Vec<PlacedTree>, n: usize, pass: usize, rng: &mut impl Rng) {
        ...
        // WEIGHT BY N: Get scaled iterations based on n importance
        let base_iterations = self.get_sa_iterations(n);
        let iterations = match pass {
            0 => base_iterations + n * 110,
            _ => base_iterations / 2 + n * 55,
        };
...
mod tests {
    ...
    #[test]
    fn test_weight_by_n_scaling() {
        let packer = EvolvedPacker::default();

        // Verify scaling is correct
        assert_eq!(packer.get_search_attempts(1), 500);
        assert_eq!(packer.get_search_attempts(10), 500);
        assert_eq!(packer.get_search_attempts(11), 350);
        assert_eq!(packer.get_search_attempts(50), 350);
        assert_eq!(packer.get_search_attempts(51), 250);
        assert_eq!(packer.get_search_attempts(100), 250);
        assert_eq!(packer.get_search_attempts(101), 180);
        assert_eq!(packer.get_search_attempts(150), 180);
        assert_eq!(packer.get_search_attempts(151), 100);
        assert_eq!(packer.get_search_attempts(200), 100);

        assert_eq!(packer.get_sa_iterations(1), 100000);
        assert_eq!(packer.get_sa_iterations(10), 100000);
        assert_eq!(packer.get_sa_iterations(11), 50000);
        assert_eq!(packer.get_sa_iterations(200), 5000);
    }
}
