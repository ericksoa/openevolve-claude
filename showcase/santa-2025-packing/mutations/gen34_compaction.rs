//! Evolved Packing Algorithm - Generation 34 COMPACTION PHASE
//!
//! MUTATION STRATEGY: Add dedicated compaction phase after SA.
//!
//! Changes from Gen28:
//! - After SA, run a greedy compaction phase
//! - Compaction only accepts strict improvements (no probabilistic acceptance)
//! - Specifically targets boundary trees to pull them inward
//! - Multiple passes of compaction until no improvement
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            elite_pool_size: 3,
            // COMPACTION parameters
            compaction_iters: 500,   // Per boundary tree
            compaction_step: 0.01,   // Step size for compaction moves
        }
    }
}
...
impl EvolvedPacker {
    pub fn pack_all(&self, max_n: usize) -> Vec<Packing> {
        ...
        for n in 1..=max_n {
            ...
            for (s_idx, &strategy) in strategies.iter().enumerate() {
                ...
                // COMPACTION PHASE: After SA, try to squeeze the packing
                self.compaction_phase(&mut trees);
    ...
    /// Compaction phase: greedily pull boundary trees toward center
    fn compaction_phase(&self, trees: &mut Vec<PlacedTree>) {
        ...
        while improved && passes < max_passes {
            ...
            for (idx, edge) in &boundary_info {
                let old_tree = trees[*idx].clone();

                // Determine direction to pull based on edge
                let (dx, dy) = match edge {
                    BoundaryEdge::Left => (self.config.compaction_step, 0.0),
                    BoundaryEdge::Right => (-self.config.compaction_step, 0.0),
                    BoundaryEdge::Top => (0.0, -self.config.compaction_step),
                    BoundaryEdge::Bottom => (0.0, self.config.compaction_step),
                    BoundaryEdge::Corner => {
                        // Pull toward center
                        let tree_cx = old_tree.x;
                        let tree_cy = old_tree.y;
                        let to_center_x = cx - tree_cx;
                        let to_center_y = cy - tree_cy;
                        let mag = (to_center_x * to_center_x + to_center_y * to_center_y).sqrt();
                        if mag > 0.001 {
                            (to_center_x / mag * self.config.compaction_step,
                             to_center_y / mag * self.config.compaction_step)
                        } else {
                            continue;
                        }
                    }
                ...
                for step in 1..=self.config.compaction_iters {
                    let new_tree = PlacedTree::new(
                        old_tree.x + dx * step as f64,
                        old_tree.y + dy * step as f64,
                        old_tree.angle_deg,
                    );

                    // Check for overlap
                    let valid = trees.iter().enumerate().all(|(i, t)| {
                        i == *idx || !new_tree.overlaps(t)
                    });

                    if valid {
                        trees[*idx] = new_tree;
                        let new_side = compute_side_length(trees);
                        if new_side < current_side - 0.0001 {
                            // Accept and continue
                            improved = true;
                        } else {
                            // Revert
