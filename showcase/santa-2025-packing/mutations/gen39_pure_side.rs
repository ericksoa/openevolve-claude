//! Evolved Packing Algorithm - Generation 39 PURE SIDE FOCUS
//!
//! MUTATION STRATEGY: Focus purely on minimizing side length.
//!
//! Changes from Gen28:
//! - Simplified scoring: primarily side length
//! - Reduced balance penalty (squares don't help much)
//! - Increased neighbor bonus (encourage tight packing)
...
impl Default for EvolvedConfig {
    fn default() -> Self {
        Self {
            ...
            gap_penalty_weight: 0.2,       // Increased
...
impl EvolvedPacker {
    ...
    /// MODIFIED: Simplified scoring focused on side length
    fn placement_score(&self, tree: &PlacedTree, existing: &[PlacedTree], _n: usize) -> f64 {
        ...
        // Simplified scoring: mostly side length
        let side_score = side;

        // Small balance penalty (reduced from 0.10 to 0.05)
        let balance_penalty = (width - height).abs() * 0.05;

        // Increased neighbor bonus (encourage tight packing)
        let neighbor_bonus = self.neighbor_proximity_bonus(tree, existing) * 1.5;

        // Extension penalty (reduced)
        let (old_min_x, old_min_y, old_max_x, old_max_y) = if !existing.is_empty() { compute_bounds(existing) } else { (0.0, 0.0, 0.0, 0.0) };
        let extension_penalty = ((pack_max_x - old_max_x).max(0.0) + (old_min_x - pack_min_x).max(0.0) + (pack_max_y - old_max_y).max(0.0) + (old_min_y - pack_min_y).max(0.0)) * 0.05;
        ...
        side_score + balance_penalty + extension_penalty + density_bonus - neighbor_bonus
    }
    ...
    fn neighbor_proximity_bonus(&self, tree: &PlacedTree, existing: &[PlacedTree]) -> f64 {
        ...
        (if min_dist < 1.5 { 0.03 * (1.5 - min_dist) } else { 0.0 }) + 0.008 * close_neighbors as f64
    }
